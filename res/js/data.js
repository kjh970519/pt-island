let islandData = [
    {
        islandName: "잠자는 노래의 섬", 
        appearTime: [
            { hour: 0, minute: 20, second: 0 },
            { hour: 2, minute: 20, second: 0 },
            { hour: 4, minute: 20, second: 0 },
            { hour: 6, minute: 20, second: 0 },
            { hour: 8, minute: 20, second: 0 },
            { hour: 10, minute: 20, second: 0 },
            { hour: 12, minute: 20, second: 0 },
            { hour: 14, minute: 20, second: 0 },
            { hour: 16, minute: 20, second: 0 },
            { hour: 18, minute: 20, second: 0 },
            { hour: 20, minute: 20, second: 0 },
            { hour: 22, minute: 20, second: 0 },
        ],
        reward: {
            mind: "잠자는 노래의 섬의 마음", 
            adventure: "모험물 : 해적의 의족", 
            heart: "", 
        }, 
        img: "sleepingsong"
    }, 
    {
        islandName: "두키 섬", 
        appearTime: [
            { hour: 0, minute: 50, second: 0 },
            { hour: 4, minute: 50, second: 0 },
            { hour: 8, minute: 50, second: 0 },
            { hour: 12, minute: 50, second: 0 },
            { hour: 16, minute: 50, second: 0 },
            { hour: 20, minute: 50, second: 0 },
        ],
        reward: {
            mind: "두키 섬의 마음", 
            adventure: "", 
            heart: "일곱 번째 거인의 심장", 
        }, 
        img: "dooky"
    }, 
    {
        islandName: "미지의 섬", 
        appearTime: [
            { hour: 0, minute: 20, second: 0 },
            { hour: 4, minute: 20, second: 0 },
            { hour: 10, minute: 20, second: 0 },
            { hour: 13, minute: 20, second: 0 },
            { hour: 16, minute: 20, second: 0 },
            { hour: 22, minute: 20, second: 0 },
        ],
        reward: {
            mind: "미지의 섬의 마음", 
            adventure: "", 
            heart: "", 
        }, 
        img: "unknown"
    }, 
    {
        islandName: "모코콩 아일랜드", 
        appearTime: [
            { hour: 0, minute: 30, second: 0 },
            { hour: 9, minute: 30, second: 0 },
            { hour: 12, minute: 30, second: 0 },
            { hour: 19, minute: 30, second: 0 },
            { hour: 21, minute: 30, second: 0 },
        ],
        reward: {
            mind: "모코콩 아일랜드 섬의 마음", 
            adventure: "", 
            heart: "", 
        }, 
        img: "mokokong_island"
    }, 
    {
        islandName: "알라케르", 
        appearTime: [
            { hour: 1, minute: 50, second: 0 },
            { hour: 7, minute: 50, second: 0 },
            { hour: 13, minute: 50, second: 0 },
            { hour: 16, minute: 50, second: 0 },
            { hour: 19, minute: 50, second: 0 },
            { hour: 22, minute: 50, second: 0 },
        ],
        reward: {
            mind: "알라케르 섬의 마음", 
            adventure: "모험물 : 달의 탑", 
            heart: "", 
        }, 
        img: "alacer"
    }, 
    {
        islandName: "환각의 섬", 
        appearTime: [
            { hour: 0, minute: 0, second: 0 },
            { hour: 2, minute: 0, second: 0 },
            { hour: 4, minute: 0, second: 0 },
            { hour: 6, minute: 0, second: 0 },
            { hour: 8, minute: 0, second: 0 },
            { hour: 10, minute: 0, second: 0 },
            { hour: 12, minute: 0, second: 0 },
            { hour: 14, minute: 0, second: 0 },
            { hour: 16, minute: 0, second: 0 },
            { hour: 18, minute: 0, second: 0 },
            { hour: 20, minute: 0, second: 0 },
            { hour: 22, minute: 0, second: 0 },
        ],
        reward: {
            mind: "환각의 섬의 마음", 
            adventure: "모험물 : 남해의 눈", 
            heart: "", 
        }, 
        img: "illusion"
    }, 
    {
        islandName: "스피다 섬", 
        appearTime: [
            { hour: 1, minute: 30, second: 0 },
            { hour: 7, minute: 30, second: 0 },
            { hour: 13, minute: 30, second: 0 },
            { hour: 19, minute: 30, second: 0 },
            { hour: 22, minute: 30, second: 0 },
        ],
        reward: {
            mind: "스피다 섬의 마음", 
            adventure: "모험물 : 해적의 깃발", 
            heart: "", 
        }, 
        img: "spida"
    }, 
    {
        islandName: "무릉도원", 
        appearTime: [
            { hour: 0, minute: 0, second: 0 },
            { hour: 6, minute: 0, second: 0 },
            { hour: 12, minute: 0, second: 0 },
            { hour: 16, minute: 0, second: 0 },
            { hour: 18, minute: 0, second: 0 },
            { hour: 22, minute: 0, second: 0 },
        ],
        reward: {
            mind: "무릉도원 섬의 마음", 
            adventure: "", 
            heart: "", 
        }, 
        img: "shangrila"
    }, 
    {
        islandName: "그릇된 욕망의 섬", 
        appearTime: [
            { hour: 1, minute: 20, second: 0 },
            { hour: 7, minute: 20, second: 0 },
            { hour: 13, minute: 20, second: 0 },
            { hour: 15, minute: 20, second: 0 },
            { hour: 19, minute: 20, second: 0 },
            { hour: 22, minute: 20, second: 0 },
        ],
        reward: {
            mind: "그릇된 욕망의 섬의 마음", 
            adventure: "", 
            heart: "", 
        }, 
        img: "isleofwrongdesire"
    }, 
];